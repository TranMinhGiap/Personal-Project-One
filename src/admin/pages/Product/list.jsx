...
import { DownloadOutlined, PlusCircleOutlined, SmileOutlined, ExclamationCircleOutlined } from '@ant-design/icons';
...
import ChangeFeatured from '../../components/common/ChangeFeatured';
...
const ProductList = () => {
    ...
    const fetchData = async () => {
      ...
      if (location.state?.success) {
        api.open({
          ...
          description: "Đã thêm 1 sản phẩm mới",
      ...
      try {
        const result = await GET("/api/v1/admin/products", params);
        console.log(result);
        setData(result.data);
        setPagination(prev => ({
          ...prev,
  ...
  const columns = [
    ...
    { 
      ...
      render: (_, record) => (
        <Tag color="default">{record.category_id.title}</Tag>
      )
    },
    { 
      title: 'Trạng thái', 
      dataIndex: 'status',
      render: (_, record) => (
        <ChangeStatusTable id={record["_id"]} status={record.status} url="products"/>
      )
    },
    { 
      title: 'Vị trí', 
    ...
    { 
      ...
      render: (_, record) => (
        record.totalStock < 15 ? (
          <Tag icon={<ExclamationCircleOutlined />} color="warning">
            {record.totalStock}
          </Tag>
        ) : (
          <Tag color="processing">{record.totalStock}</Tag>
        )
      )
    },
    {
